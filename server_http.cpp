...
void sendbackHTTP(int newSd, std::string message, std::string state,  int max) {
	std::string header = "HTTP/1.1 "+ state +"\r\n" +
	"Access-Control-Allow-Origin: *\r\n\r\n";
	sendback(newSd, header + message, MAXNUM_CHAR);
}
...
void* handleClient(void* arg){
	...
	buffer.clear();
    // cout << "Received by server: " << input << endl;
	// shutdown(newSd, SHUT_RDWR);
    // pthread_exit(NULL);

    std::vector<std::string> cmdList;
    //parser the command
    cmdList = parseHttpCmd(input);
	// debug
	// cout << "parsed content in cmdList (size is "<<cmdList.size()<<")" << endl;
    ...
    if(cmdList.size() < 1) {
        sendbackHTTP(newSd, "Server can not resolve http content!", "400 Bad Request", MAXNUM_CHAR);
        shutdown(newSd, SHUT_RDWR);
        pthread_exit(NULL);
    }
    ...
    if(b){ //check first command when server argument without -r
        if(cmdList.size()!=2 || cmdList[0].compare("start_election")!= 0 || cmdList[1].compare(pw)!= 0){ //check the command type
			sendbackHTTP(newSd, "Start_election has to be the first command to execute!",
			"405 Method Not Allowed", MAXNUM_CHAR);
    ...
    if(cmdList[0].compare("shutdown") == 0){
        ...
        if(cmdList.size()!=2 || cmdList[1].compare(pw)!= 0 || serverend){
			sendbackHTTP(newSd, "password is invalid!",
			"400 Bad Request", MAXNUM_CHAR);
    ...
    std::string response;
	cout << "get response "<<response<< " correctly"<<endl;
    response = handleCmd(cmdList);
    // sendback(newSd, response, MAXNUM_CHAR);
	sendbackHTTP(newSd, response, "200 OK", MAXNUM_CHAR);
    shutdown(newSd, SHUT_RDWR);
    pthread_exit(NULL);
...
// 	int BACKUP_INTERVAL = 20;
    ...
    while(true){
		...
		// sendback(newSd.back(), "HTTP/1.1 200 OK\r\nAccess-Control-Allow-Origin: *\r\n\r\nHello", MAXNUM_CHAR);
		// shutdown(newSd.back(), SHUT_RDWR);
    ...
    if(shutdownserver().compare("OK")!=0){ //
		sendbackHTTP(shutsd, "server is unable to shutdown now",
			"500 Internal Server Error", MAXNUM_CHAR);
        //sendback(shutsd,"ERROR", MAXNUM_CHAR);
        shutdown(shutsd, SHUT_RDWR);
        shutdown(sd, SHUT_RDWR);
        return -1;
    }else{
		sendbackHTTP(shutsd, "OK", "200 OK", MAXNUM_CHAR);
